...
export async function analyzeResume(text: string, industry: keyof typeof INDUSTRY_KEYWORDS): Promise<AIAnalysisResult> {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: \`You are a professional resume reviewer. Given a resume, provide a score (0–100), a summary analysis, specific suggestions, and identify any skill gaps based on the \${industry} industry.\`
      },
      {
        role: "user",
  ...
  if (!result) throw new Error("AI did not return a response.");
